...
`git clone https://github.com/ByteLegendQuest/java-fix-increment-bug`

A project directory named `java-fix-increment-bug` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-increment-bug` directory which is created just now.
